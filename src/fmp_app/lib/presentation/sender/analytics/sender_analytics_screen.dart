...
class SenderAnalyticsScreen extends StatefulWidget {
  const SenderAnalyticsScreen({Key? key}) : super(key: key);

  @override
  State<SenderAnalyticsScreen> createState() => _SenderAnalyticsScreenState();
}

class _SenderAnalyticsScreenState extends State<SenderAnalyticsScreen> {
  SenderAnalyticsModel? _data;
  ...
  Future<void> _fetchData() async {
    try {
      ...
      final data = await source.getSenderAnalytics();
      if (mounted) {
        setState(() {
          _data = data;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Logistics Analytics')),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            AppLineChart(title: 'Logistics Spend (Last 7 Days)', data: _data!.logisticsSpend),
            const SizedBox(height: 24),
            AppPieChart(title: 'My Shipments Status', data: _data!.shipmentStatusBreakdown),
          ],
        ),
      ),
    );
